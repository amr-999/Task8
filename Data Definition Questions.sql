...
ADD CONSTRAINT FK_Projects_Employees FOREIGN KEY (ProjectID) REFERENCES Employees(ID);
...
-- 10. Create a table named "Customers" with columns for CustomerID, FirstName, LastName, Email, and Status.
...
-- 13. Create a table named "Orders" with columns for OrderID, CustomerID, OrderDate, and TotalAmount.
CREATE TABLE Orders (
    OrderID     INT,
    CustomerID  INT,
    OrderDate   DATETIME,
...
-- 14. Add a check constraint to the "TotalAmount" column to ensure it is greater than zero.
ALTER TABLE Orders
